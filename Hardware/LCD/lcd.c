 ...
 * @param       int_len    : 整数部分位数（如123.45的整数位为3）
 * @param       dec_len    : 小数部分位数（如123.45的小数位为2）
 * @param       size       : 字体大小（像素高度，支持12/16/24/32）
 * @param       color      : 显示颜色（RGB565格式）
 * @retval      无
 */
void lcd_show_double(uint16_t x, uint16_t y, double num, uint8_t int_len, uint8_t dec_len, uint8_t size, uint16_t color) {
    uint8_t t, temp;
    uint8_t enshow = 0;
    uint32_t int_part;
    uint32_t dec_part;
    double abs_num = (num < 0) ? -num : num;  // 处理负数

    // 1. 显示负号（若为负数）
    if (num < 0) {
        lcd_show_char(x, y, '-', size, 0, color);
        x += size / 2;  // 负号占一位，调整后续显示位置
    }

    // 2. 提取整数部分和小数部分
    int_part = (uint32_t)abs_num;
    dec_part = (uint32_t)((abs_num - int_part) * lcd_pow(10, dec_len) + 0.5);  // +0.5实现四舍五入

    // 3. 显示整数部分（支持前导零抑制）
    for (t = 0; t < int_len; t++) {
        temp = (int_part / lcd_pow(10, int_len - t - 1)) % 10;
    ...
    // 4. 显示小数点
    lcd_show_char(x + (size / 2) * int_len, y, '.', size, 0, color);

    // 5. 显示小数部分（不足位补零）
    for (t = 0; t < dec_len; t++) {
        temp = (dec_part / lcd_pow(10, dec_len - t - 1)) % 10;
        lcd_show_char(x + (size / 2) * (int_len + 1 + t), y, temp + '0', size, 0, color);
    }
}
